...
| BOUND-BID-05 | Player bid above opponent | **No known-answer coverage exists** — see Unresolved gaps | Player wins and pays the submitted bid (`DEMO_SPEC.md` §11, DEC-006) | Cannot be assigned a numeric pass rule until a FIN-EX example or explicit team decision provides one | Not Yet Executed — blocked |
...
- **BOUND-BID-05 (player bid above opponent) has no known-answer coverage.** `docs/finance/source-ledger.md` provides FIN-EX-007 (below) and FIN-EX-008 (tied) but no example where the player's bid exceeds the opponent's and the win/cash outcome is derived numerically. This must be filled with a new FIN-EX example or an explicit team decision before the test can be assigned a concrete expected value.
- Sector return rates, indicator-to-sector mapping, and permitted sector-rate ranges remain unresolved (`DEMO_SPEC.md` §16).
- Starting cash, property name/type/asking price, gross property income/cost, and the scripted opponent bid amount remain Provisional placeholders only (`DEMO_SPEC.md` §16; `docs/DECISIONS.md` DEC-008).
- Coach financial terminology remains unapproved (`DEMO_SPEC.md` §16).
