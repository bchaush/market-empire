...
- No application (React/TypeScript) code exists in this repository yet. Every test below is therefore **Not Yet Executed**. No test in this document may be marked as passed until implementation exists and the test has actually been run against it, per `AGENTS.md`.
  - **Evidence update (2026-07-30):** Application code now exists, and a subset of the tests below has since been executed. The individual Status cells in this document are preserved unchanged as the original plan's historical record and are not edited here. For current, dated, per-ID pass/fail results and their exact evidence, see `docs/testing/evidence/issue-11-final-acceptance.md`. Several requirements — particularly the Manual- and Accessibility-typed ones — remain genuinely Not Yet Executed as a formal human pass.
...
- No application code exists in this repository yet. Every test in this plan is Not Yet Executed, and none may be marked passed until implementation exists and the test is actually run, per `AGENTS.md`.
  - **Evidence update (2026-07-30):** see `docs/testing/evidence/issue-11-final-acceptance.md` for the current, dated, per-ID evidence record — this bullet's original text is preserved unchanged as history and is not itself updated.
- **BOUND-BID-05 (player bid above opponent) now has known-answer coverage.** `docs/finance/source-ledger.md` FIN-EX-010, added via `docs/DECISIONS.md` DEC-010 (2026-07-30), provides a concrete winning-bid example (`player_bid=4500`, `opponent_bid=4250`, `property_won=true`, `winning_bid=4500`, `net_property_result=250`, `cash_after_property=6550`). The corresponding Vitest test has not yet been implemented or executed — status remains Not Yet Executed.
  - **Evidence update (2026-07-30):** the corresponding Vitest test now exists (`src/engine/property.test.ts`, `BOUND-BID-05 (FIN-EX-010)`) and passes; see `docs/testing/evidence/issue-11-final-acceptance.md`. The bullet above is preserved unchanged as history.
